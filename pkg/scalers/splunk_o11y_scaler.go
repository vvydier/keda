...
func NewSplunkO11yScaler(ctx context.Context, config *ScalerConfig) (Scaler, error) {
...
func parseSplunkO11yMetadata(config *ScalerConfig, logger logr.Logger) (*splunkO11yMetadata, error) {
	...
	if metricName, ok := config.TriggerMetadata["metricName"]; ok {
		meta.metricName = GenerateMetricNameWithIndex(config.ScalerIndex, kedautil.NormalizeString(fmt.Sprintf("signalfx-%s", metricName)))
	} else {
...
func newSplunkO11yConnection(ctx context.Context, meta *splunkO11yMetadata, config *ScalerConfig) (*signalflow.Client, error) {
